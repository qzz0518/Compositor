...
final class ProjectController {
    ...
    func exportPNG() async {
        ...
        catch { await showError(String(localized: "Couldn’t export PNG"), error: error) }
    }
    ...
    func canvasSize() async {
        ...
        } catch { await showError(String(localized: "Couldn’t change canvas size"), error: error) }
    }
    ...
    func imageSize() async {
        ...
        } catch { await showError(String(localized: "Couldn’t resize the image"), error: error) }
    }
    ...
    func exportJPEG() async {
        ...
        } catch { await showError(String(localized: "Couldn’t export JPEG"), error: error) }
    }
    ...
    func open(_ suppliedURL: URL? = nil) async -> Bool {
        ...
        do {
            ...
            await showError(String(localized: "Couldn’t open the project"), error: error)
            return false
        }
    }
    ...
    private func drainIncoming() async {
        while !incoming.isEmpty {
            ...
            if projects.count > 1 {
                await showError(String(localized: "Open one project at a time"), error: ProjectError.invalid)
            } else {
                var proceed = true
